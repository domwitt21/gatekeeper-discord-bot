...
class DashboardServer {
    ...
    routes() {
        ...
        this.app.post("/guild/:guildId/moderation", this.requireGuild.bind(this), this.requireCsrf.bind(this), async (request, response, next) => {
            try {
                ...
                if (!["verify", "unverify", "reset"].includes(action)) throw new Error("Invalid moderation action.");
                if (["unverify", "reset"].includes(action) && request.body.confirmation !== "on") throw new Error("Confirm this action before continuing.");
    ...
    async overview(guildId, filters = {}) {
        const [successes, failures, recent, settings, daily, failureReasons, securityEvents, trustPolicies, reportDeliveries] = await Promise.all([
    ...
    async guildPageV2(request) {
        ...
        const notice = request.query.saved ? '<div class="notice">Settings saved and the verification message was republished.</div>' : request.query.policySaved ? '<div class="notice">Trust policy saved.</div>' : request.query.moderated ? '<div class="notice">Moderation action completed.</div>' : "";
        ...
        <nav class="section-nav" aria-label="Dashboard sections"><a href="#overview">Overview</a><a href="#configuration">Configuration</a><a href="#reports">Reports</a><a href="#policies">Trust policies</a><a href="#moderation">Moderation</a><a href="#message">Message</a><a href="#activity">Activity</a></nav>
