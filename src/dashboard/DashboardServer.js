...
const SecurityExportService = require("../services/SecurityExportService");
...
class DashboardServer {
    ...
    routes() {
        ...
        this.app.get("/guild/:guildId/analytics.csv", this.requireGuild.bind(this), async (request, response, next) => {
            try {
                const analytics = new AnalyticsService(this.client);
                const range = analytics.range(this.analyticsOptions(request.query));
                const rows = await analytics.exportRows(request.dashboardGuild.id, range, request.query.identifiers !== "raw");
                const csv = new SecurityExportService().csv(rows);
                await this.client.database.securityEvents.record({ guildId: request.dashboardGuild.id, type: "ANALYTICS_CSV_EXPORTED",
                    details: `${rows.length} rows; privacy ${request.query.identifiers === "raw" ? "raw" : "safe"}; by ${request.session.user.id}` });
                response.set({ "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="sentraguard-${request.dashboardGuild.id}-analytics.csv"`, "Cache-Control": "no-store" }).send(csv);
            } catch (error) { next(error); }
        });
        this.app.get("/guild/:guildId/security-report.pdf", this.requireGuild.bind(this), async (request, response, next) => {
            try {
                const analytics = new AnalyticsService(this.client);
                const report = await analytics.generate(request.dashboardGuild.id, this.analyticsOptions(request.query));
                const pdf = await new SecurityExportService().pdf(request.dashboardGuild, report);
                await this.client.database.securityEvents.record({ guildId: request.dashboardGuild.id, type: "SECURITY_PDF_EXPORTED",
                    details: `${report.range.days} days; by ${request.session.user.id}` });
                response.set({ "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="sentraguard-${request.dashboardGuild.id}-security-report.pdf"`, "Cache-Control": "no-store" }).send(pdf);
            } catch (error) { next(error); }
        });
    ...
    parseSettings(body, userId) {
        ...
        return {
            ...
            reportFrequency: ["DAILY", "MONTHLY"].includes(body.reportFrequency) ? body.reportFrequency : "WEEKLY",
            ...
            dataRetentionDays: integer(body.dataRetentionDays, 0, 3650, 0),
    ...
    wizardSettings(body, previous, userId) {
        ...
        return {
            ...
            reverifyAfterDays: Number(previous.reverify_after_days) || 0, dataRetentionDays: Number(previous.data_retention_days) || 0,
            reverificationEnforcementEnabled: Number(previous.reverification_enforcement_enabled) === 1,
    ...
    analyticsOptions(query = {}) {
        const days = Math.min(Math.max(Number.parseInt(query.days, 10) || 30, 1), 365);
        const parseDate = value => /^\d{4}-\d{2}-\d{2}$/.test(value || "") ? Date.parse(`${value}T00:00:00Z`) : null;
        const startMs = parseDate(query.start);
        const parsedEnd = parseDate(query.end);
        return startMs && parsedEnd && parsedEnd >= startMs ? { startMs, endMs: parsedEnd + 86400000, days } : { days };
    }
    ...
    async guildPageV2(request) {
        ...
        const analyticsReport = await new AnalyticsService(this.client).generate(guild.id, this.analyticsOptions(request.query));
        ...
        const analyticsPeak = Math.max(1, ...analyticsReport.daily.map(row => Number(row.successes) + Number(row.failures)));
        const analyticsBars = analyticsReport.daily.map(row => `<div class="analytics-bar" title="${escapeHtml(row.day)}: ${Number(row.successes)} successful, ${Number(row.failures)} failed"><i style="height:${Math.max(3, (Number(row.successes) + Number(row.failures)) / analyticsPeak * 100)}%"></i><span>${escapeHtml(String(row.day).slice(5))}</span></div>`).join("") || '<p class="empty-state">No activity in this period.</p>';
        const analyticsFailures = analyticsReport.failures.map(row => `<li><span>${escapeHtml(row.reason)}</span><strong>${row.count}</strong></li>`).join("") || '<li><span>No failures</span><strong>0</strong></li>';

        return `${notice}
        <section class="guild-heading"><div><a href="/">← Servers</a><p class="eyebrow">SentraGuard</p><h1>Verification overview</h1></div><span class="live ${highAlertActive ? "alert" : ""}"><i></i> ${escapeHtml(raidStatus)}</span></section>
        <div class="dashboard-menu"><button id="dashboardMenuButton" class="cyber-menu-button" type="button" aria-expanded="false" aria-controls="dashboardTabs"><span class="cyber-menu-icon" aria-hidden="true"><i></i><i></i><i></i><b></b></span><span class="menu-copy"><small>SentraGuard console</small><strong>Navigation</strong></span><em>OPEN</em></button><nav id="dashboardTabs" class="dashboard-tabs" role="tablist" aria-label="Dashboard sections" hidden><button type="button" role="tab" data-dashboard-tab="overview"><span>01</span>Overview</button><button type="button" role="tab" data-dashboard-tab="settings"><span>02</span>Settings</button><button type="button" role="tab" data-dashboard-tab="reverification"><span>03</span>Reverification</button><button type="button" role="tab" data-dashboard-tab="onboarding"><span>04</span>Onboarding</button><button type="button" role="tab" data-dashboard-tab="policies"><span>05</span>Policies</button><button type="button" role="tab" data-dashboard-tab="moderation"><span>06</span>Moderation</button><button type="button" role="tab" data-dashboard-tab="analytics"><span>07</span>Analytics</button><button type="button" role="tab" data-dashboard-tab="activity"><span>08</span>Activity</button></nav></div>
        ...
        </div><form id="configuration" class="settings-form dashboard-panel" data-dashboard-panel="settings" data-report-frequency="${escapeHtml(settings.report_frequency || "WEEKLY")}" data-retention-days="${Number(settings.data_retention_days) || 0}" method="post" action="/guild/${guild.id}/settings" hidden><input type="hidden" name="csrf" value="${request.session.csrf}">
