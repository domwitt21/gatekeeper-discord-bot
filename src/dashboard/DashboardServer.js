    ...
    routes() {
        ...
        this.app.get("/guild/:guildId/analytics.csv", this.requireGuild.bind(this), async (request, response, next) => {
            try {
                ...
                response.set({ "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="gatekeeper-${request.dashboardGuild.id}-analytics.csv"`, "Cache-Control": "no-store" }).send(csv);
            } catch (error) { next(error); }
        });
        this.app.get("/guild/:guildId/security-report.pdf", this.requireGuild.bind(this), async (request, response, next) => {
            try {
                ...
                response.set({ "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="gatekeeper-${request.dashboardGuild.id}-security-report.pdf"`, "Cache-Control": "no-store" }).send(pdf);
            } catch (error) { next(error); }
        });
